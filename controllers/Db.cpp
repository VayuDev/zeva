...
void Api::Db::getAllTables(const drogon::HttpRequestPtr&,
...
void Api::Db::getTableCsv(const drogon::HttpRequestPtr&req, std::function<void(const drogon::HttpResponsePtr &)> &&callback,
                     std::string &&pName) {
    bool truncateTimestamps = false;
    auto truncateTimestampsParm = req->getParameters().find("truncateTimestamps");
    if(truncateTimestampsParm != req->getParameters().end() && truncateTimestampsParm->second == "true") {
        truncateTimestamps = true;
    }
    bool skipId = false;
    auto skipIdParam = req->getParameters().find("skipId");
    if(skipIdParam != req->getParameters().end() && skipIdParam->second == "true") {
        skipId = true;
    }
    ...
    drogon::app().getDbClient()->execSqlAsync(
        ...
        [callback=std::move(callback), skipId, truncateTimestamps, tablename = pName](const drogon::orm::Result& result) {
