...
public:
  ...
  query(std::string pQuery, std::vector<QueryValue> pPlaceholders = {}) = 0;
  void addListener(const std::string &pChannel,
                   std::function<void(const std::string &)> pListener);
  virtual void awaitNotifications(int millis) { (void)millis; };

protected:
  virtual void listenTo(const std::string &pChannel) = 0;
  virtual void onNotification(const std::string &pChannel,
                              const std::string &pPayload);

private:
  std::list<std::pair<std::string, std::function<void(const std::string &)>>>
      mListeners;
