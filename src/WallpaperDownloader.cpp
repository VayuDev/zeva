    ...
    client->sendRequest(req, [](drogon::ReqResult result, const drogon::HttpResponsePtr& resp) {
        ...
        for (const auto &post: json) {
            if (post["data"]["ups"].asInt64() >= 100) {
                ...
                try {
                    //ugly way to get something like https://i.reddit.it from a full request url
                    //this will fail horribly if the string isn't formatted correctly
                    //that's why we have the try-catch
                    auto thirdSlashIndex = urlStr.find('/', urlStr.find('/', urlStr.find('/') + 1) + 1);
